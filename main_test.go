...
func TestSetMaze(t *testing.T) {
	tests := []test{
		...
		{"InvalidLinkToItself", "examples/badexample01.txt", 20, true}, // invalid link 3-3
		{"InvalidLinkChar", "tests/bad_2dashes.txt", 4, true},          // invalid link 2--3
		// {"InvalidRoomsOrLinks", "tests/bad_3S2E.txt", 4, true},                // >1 ##start/##end
		{"InvalidRoomsNo##end", "tests/badexample_noEnd.txt", 1, true},             // no ##end
		{"InvalidRoomsNo##start", "tests/badexample_noStart.txt", 1, true},         // no ##start
		{"InvalidRoomsNoEndRoom", "tests/badexample_noEndValue.txt", 4, true},      // no end room (line after ##end is invalid)
		{"InvalidRoomsNoStartRoom", "tests/badexample_noStartValue.txt", 4, true},  // no start room (line after ##start is invalid)
		{"InvalidRoomsSameStartEnd", "tests/badexample_sameStartEnd.txt", 4, true}, // start room name = end room name
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
